...
export type RuntimeFunction<Request = unknown, Response = unknown, Next = unknown> = (
	...
	const fragmentReq = new Request(upstreamUrl, {
		...
		headers: new Headers(
			(function () {
				...
				newHeaders.set(
					'Accept-Encoding',
					mode === 'development' ? 'gzip' : request.headers.get('Accept-Encoding') || '',
				);
...
export function handleFetchErrors(fragmentConfig: FragmentConfig, fragmentRequest: Request) {
	return async (failedResOrError: unknown) => {
		const { upstream, onSsrFetchError = defaultErrorHandler } = fragmentConfig;
...
export function defaultErrorHandler(error: unknown): Promise<{ response: Response; overrideResponse: boolean }> {
	return Promise.resolve({
		response: new Response(`<p>Error fetching fragment: ${error}</p>`, { headers: { 'content-type': 'text/html' } }),
